 ...
 */
// DIVERGENCE FROM w3spay: `terminal` removed from the deny-list. Terminal IDs
// in this product are public receipt-QR labels (printed verbatim in QR codes,
// e.g. "955002-00") and per-terminal granularity is the whole point of this
// instrumentation. Everything else stays — notably `amount`: amounts never
// leave the device (local records only).
export const SENSITIVE_KEY_RE =
  /destination|merchant|payment_?id|tx_?hash|amount|kassen|raw|address|account|signer|wallet|public_?key|secret|email|phone|user_?id/i;
