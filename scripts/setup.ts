/**
 * Guided one-command deploy for w3s-payment-processor.
 *
...
async function phaseConfigure(flags: SetupFlags): Promise<Config> {
  ...
  if (domainInput) domain = normalizeDomain(domainInput);
    ...
    domain = normalizeDomain(
      await ui.text("Target .dot domain", {
        validate: (v) => (v.trim() ? null : "Enter a domain, e.g. yourproduct.dot"),
      }),
    );
  }
  ...
  if (!flags.dryRun) {
    ...
    upsertEnvFile(ENV_LOCAL, values, {
      headerComment: "# Local-only env for w3s-payment-processor. Gitignored — never commit secrets.",
    });
  }
...
async function main(): Promise<void> {
  ...
  ui.blank();
  ui.log(ui.c.bold("w3s-payment-processor deploy"));
